...
opt_parser <- list(
  make_option("--experiment", type = "character", default = NULL,
              ...
              metavar = "character"),
  make_option("--smooth_dir", type = "character", default = NULL,
              help = "Name of a smooth directory e.g. 'smooth_3x3_n10'",
              metavar = "character"),
...
out_dir    <- opt$out_dir    # "/net/150.163.2.206/disks/d6/shared/alber/prodes_reproduction/03_classify/rep_prodes_40/results_vote"
cores      <- opt$cores      # 8
...
stopifnot(all(vapply(c(out_dir, base_path, dl_dir, rf_dir, svm_dir), dir.exists, logical(1))))
...
result_tb <- function(path){
  path %>%
    ...
    ensurer::ensure_that(length(.) > 0,
                         err_desc = sprintf("No classified images found in %s",
                                            path)) %>%
...
for (i in seq_along(raster_mode_ls)) {
  fname <- raster_tb %>%
    dplyr::select(dl_file) %>%
    dplyr::slice(i) %>%
    unlist() %>%
    basename() %>%
    stringr::str_replace("dl", "dl-rf-svm") %>%
    stringr::str_replace(".tif", '')
  print(sprintf("Writting result to %s",
                file.path(out_dir, paste0(fname, "_vote.tif"))))
  raster::writeRaster(raster_mode_ls[[i]],
                      file.path(out_dir, paste0(fname, "_vote.tif")))
  raster::writeRaster(raster_mode_freq_ls[[i]],
                      file.path(out_dir, paste0(fname, "_vote-freq.tif")))
}
