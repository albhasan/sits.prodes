...
smooth_dir <- c(NA, "smooth_11x11_n10", "smooth_3x3_n10", "smooth_5x5_n10", "smooth_7x7_n10", "smooth_9x9_n10")
...
tests <- diff_files %>% dplyr::select(experiment, smooth, pyear, scene) %>%
...
tests %>% dplyr::select(common_confusion, filename) %>%
    purrr::pmap(., function(common_confusion, filename){raster::writeRaster(common_confusion, filename = filename)})
