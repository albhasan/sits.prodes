...
result_path <- file.path(base_path, "03_classify", stringr::str_replace(train, "train", "rep_prodes"), "results_rf")
...
for(path_row in sort(unique(brick_tb$pathrow))){
  for(y in sort(unique(brick_tb$year))){
    ...
    result_filepath <- file.path(result_path, paste(coverage_name, "rf", sep = "_"))
