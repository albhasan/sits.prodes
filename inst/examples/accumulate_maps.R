...
opt_parser <- list(
  ...
  make_option("--out_dir",    type = "character", default = NULL,
              help = "Path to a directory where to store the results.",
              metavar = "character")
) %>%
  OptionParser()
...
if (!all(vapply(c(in_dir, base_path, corner_mask_path), dir.exists, logical(1)))) {
  print_help(opt_parser)
...
in_path <- in_dir %>%
  list.files(pattern = img_pattern, full.names = TRUE, include.dirs = FALSE) %>%
...
img_tb <- in_path %>%
  ...
  dplyr::as_tibble() %>%
  dplyr::select(satellite = V1, brick = V2, scene = V3,
...
get_prodes_map <- function(img_path, out_file){
  ...
  # mask the results
  res <- corner_mask_path %>%
    file.path(paste0("LC08_CORNERMASK_", scene,"_2016.tif")) %>%
    c(last_img) %>%
...
for(sc in unique(img_tb$scene)) {
  ...
  out_file <- out_dir %>%
    file.path(sprintf("prodes_reproduction_%s_%s.tif", sc, prodes_year))
