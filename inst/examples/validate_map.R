...
corner_masks <- base_path %>% file.path("data", "raster", "mask_l8_corner") %>%
  ...
  dplyr::mutate(fname = basename(file_path),
                scene   = stringr::str_extract(fname, pattern = "_[0-9]{6}[.]") %>%
                  stringr::str_sub(2, -2))
...
res_acc <- purrr::map(path_res_vec, function(res_file, out_dir = NULL){
  ...
  # Computing a confusion coverage. The most significant one or two digits
  # are the referece id while the two least significant are the result id.
