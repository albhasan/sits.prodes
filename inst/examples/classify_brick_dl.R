...
opt_parser <- list(
  ...
  make_option("--model", type = "character", default = NULL,
              help = "Name of a trained model e.g. 'train_13_model_8'",
              metavar = "character"),
  ...
  make_option("--btype", type = "character", default = NULL,
              help = "Type to brick to use. The alternatives are 'mod13', 'l8mod_interp', and 'l8mod_starfm'",
              metavar = "character"),
...
stopifnot(all(vapply(c(base_path, path_to_bricks), dir.exists, logical(1))))
...
model_fpaths <- file.path(base_path, "02_train_model", train,
                          paste0(sits_model, ".", smodel_files))
...
training_labels <- dl_model %>%
  environment() %>%
  .[["int_labels"]]
training_bands <- dl_model %>%
  environment() %>%
  .[["data.tb"]] %>% sits::sits_bands()
write.csv(
...
brick_tb <- path_to_bricks[brick_type] %>%
  ensurer::ensure_that(!is.na(.),
                       err_desc = sprintf("Invalid type of brick: %s", brick_type)) %>%
