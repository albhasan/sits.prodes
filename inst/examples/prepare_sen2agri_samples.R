...
read_prodes <- function(x){
    ...
    legend_tb <- "/home/alber/Documents/ghProjects/sits.prodes/inst/extdata/esa_cci_lc_legend.csv" %>%
        readr::read_csv() %>%
        dplyr::mutate(label = ifelse(is.na(Global_label), Regional_label, Global_label),
                      value = ifelse(is.na(Global_value), Regional_value, Global_value)) %>%
        dplyr::select(sen2agri_label = label, sen2agri_id = value) %>%
        dplyr::mutate(CROP = as.integer(stringr::str_detect(sen2agri_label, "Cropland")),
                      LC = stringr::str_sub(sen2agri_label, 1, 70),
                      CODE = as.integer(sen2agri_id),
                      IRRIGATION = 0L) %>%
        ensurer::ensure_that(!any(sapply(., is.na)), err_desc = "Inconsistent legend.")
...
sentinel_tb <- sentinel_tiles %>%
    ...
    dplyr::mutate(bounding_box = purrr::map(sf, get_bbox),
                  ...
                  out_shp = file.path(out_dir, stringr::str_c("BR_", tile , "_LC_II_201911.shp"))) %>%
    # Format the PRODES data to fit the Sen2agri's sample schema.
    dplyr::mutate(samples_sf = purrr::map2(.$prodes, .$out_shp, function(prodes, out_shp){
        prodes %>%
